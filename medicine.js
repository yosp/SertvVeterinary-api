...
hash.set('GET /:labid', async function getMedicine (req, res, param) {
  let labid = param.labid
  await db.connect()
  let med = await db.getMedicineByLab(labid)
  await db.disconnect()
  send(res, 200, med)
})

hash.set('POST /', async function saveMedicine (req, res, param) {
  let med = await json(req)
  await db.connect()
  let created = await db.saveMedicine(med)
  await db.disconnect()
...
hash.set('POST /update', async function updateLab (req, res, param) {
	...
	await db.connect()
	let updated = await db.updateMedicine(med)
	await db.disconnect()
